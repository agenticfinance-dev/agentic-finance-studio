...
async def load_symbols(session):
    ...
    try:
        ...
        logging.info(f"[SoDEX] Loading symbols from {url}")

        async with session.get(url) as r:
            txt = await r.text()
            logging.info(f"[SoDEX] Status: {r.status}")
            logging.info(f"[SoDEX] Response: {txt[:1000]}")

            if r.status!= 200:
                logging.warning(f"[SoDEX] Failed to load symbols - status {r.status}")
                return
        ...
        logging.exception(f"[SoDEX] load_symbols exception: {e}")
...
class SoDEXExecutor:
    ...
    async def place_order(self, session, symbol: str, bias: str, entry: float, qty: float):
        ...
        if symbol_id is None: return {"err": f"symbolID not loaded for {symbol_str}. Loaded: {SYMBOL_IDS}"}

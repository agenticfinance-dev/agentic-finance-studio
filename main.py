...
# FIXED: Reuse same session
async def main():
    await start_webserver()

    # Create ONE session for everything
    ...
    try:
        ...
        logging.warning(f"Startup tasks failed: {e}")

    app = ApplicationBuilder().token(TOKEN).build()
    app.bot_data["session"] = shared_session # Reuse same session
